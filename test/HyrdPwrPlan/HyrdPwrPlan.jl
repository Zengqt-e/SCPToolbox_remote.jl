...
LmdG = 0.06;    # 0.06Wb
weG = 3e3/60*4*2*pi; #3krpm
wmM = 1e3/60*2*pi;   #1krpm

f(t, x, u, p) = begin       # c is constant/parameters in system
    udc = x;
    iqG, tqM = u;
    return -1/Cap*(1.5*iqG*weG*LmdG + tqM*wmM)*udc^(-1)*t_f;
end

A(t, x, u, p) = begin    # A*δx
    udc = x;
    iqG, tqM = u;
   return -1/Cap*(1.5*iqG*weG*LmdG + tqM*wmM)*(-1)*udc^(-2)*t_f;
end

B(t, x, u, p) =begin     # B*δu = B*δ(iqG;tqM)
    udc = x;
    iqG, tqM = u;
    return  -1/Cap*[1.5*weG*LmdG, wmM]*udc^(-1)*t_f;
end
...
wrap(func) = (t, x, u, p, pbm) -> func(t, x, u, p);
...
R(x, u ,p) = u'*u;
wrap(func) = (t, x, u, p, c, pbm) -> func(x, u, p);
